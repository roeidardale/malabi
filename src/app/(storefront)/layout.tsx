...
export default async function StorefrontLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <>
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3">
          ...
          <nav className="flex items-center gap-2 sm:gap-3">
            <a
              ...
              className="hidden items-center gap-2 rounded-md border border-border px-3 py-2 text-sm text-foreground transition-colors hover:border-accent sm:flex"
            >
              הזמנה בוואטסאפ
            </a>
            ...
            >
              <span>סל קניות</span>
              {itemCount > 0 && (
                <span className="inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-accent px-1 text-xs font-bold text-black">
                  {itemCount}
                </span>
              )}
            </Link>
      ...
      <footer className="border-t border-border bg-surface-deep px-4 py-6 text-center text-sm text-muted">
