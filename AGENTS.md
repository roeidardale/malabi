...
# Malabi — agent guidance

Malabi is a local-only, functional rebuild of malabi-expres.co.il (authorized by the site owner): a Hebrew/RTL storefront + admin/dispatch panel for a delivery business in Ashkelon, Israel. Not deployed. `docs/ROADMAP.md` (imported via `docs/README.md`'s index) is the authoritative "what's next" list — check it before starting new work, and update it / `FEATURES.md` / `KNOWN-ISSUES.md` / `CHANGELOG.md` per the workflow it describes.
