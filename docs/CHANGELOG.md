...
## Real price import + single-option products (2026-09-23)
- **`scripts/import-prices.ts` now creates variants, not just prices**: previously it only updated the price of a variant that already existed with a matching name, so it silently skipped every product that had zero variants — the exact thing making the catalog unbuyable (GitHub #3). It now creates a variant (named from the CSV row, `priceSource="IMPORTED"`) for any matched product that started with none; a product that already has variants and just doesn't have one matching the row's name is still left alone and reported (that's a naming/data-quality question for a human, not something to guess at).
- **Ran against `malabi-price-report-final.csv`** (`--apply`): 115 variants created for previously-unbuyable products, 82 existing ₪0 variants given real prices. 288 products remain without a variant — those are whole product/categories the scraper never picked up at all (GitHub #4), a separate, bigger gap than a price import can fix.
- **Single-variant products no longer show a pointless picker**: `AddToCart` used to only hide the variant chips/select when the lone variant's name was exactly the `DEFAULT_VARIANT_NAME` sentinel ("יחידה") — a real single size like `"500 מ״ל"` still rendered a one-option radio group. It now hides the picker whenever there's only one variant, period, and shows that variant's name as plain text underneath the price only when it's not the generic placeholder.
- **Admin: no more inventing a variant name for a single-price product**: adding a product's *first* variant now shows a "single option" checkbox (checked by default) that submits the `DEFAULT_VARIANT_NAME` sentinel instead of requiring the admin to type something. Unchecking it reveals a normal free-text name field for when the product actually has sizes. Once a product has 2+ variants the normal per-row name field is used as before.
